...
IPPortPatternLine = re.compile(
    r'^.*?(?P<ip>(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)).*?(?P<port>\d{1,5}).*$',
    # noqa
    flags=re.MULTILINE)

IPPortPatternGlobal = re.compile(
    r'(?P<ip>(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?))'  # noqa
    r'(?=.*?(?:(?:(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?))|(?P<port>\d{1,5})))',
    # noqa
    flags=re.DOTALL)
...
def get_self_ip():
    global _cached_ip
    if not _cached_ip:
        _cached_ip = json.loads(urlopen('http://httpbin.skactor.tk:8080/ip')
                                .read().decode('utf-8'))['origin']
    return _cached_ip
...
def ip_location(ip):
    ...
    ret = {'ret': None}
    if ret['ret'] != 'ok':
        ip_loc = ipdb.City(os.path.join(Config.PROJECT_DIR, 'utils', 'ipdb',
                                        'mydata4vipday1.ipdb'))
