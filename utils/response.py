...
from utils._parser import HTTPResponseParser
...
class HTTPResponse(HTTPResponseParser):
    ...
    def should_close(self):
        ...
        """
        return not self.message_complete or self.parser_failed() or not super(HTTPResponse, self).should_keep_alive()
    ...
    def _flush_header(self):
        if self._current_header_field is not None:
            self._headers_index[self._current_header_field] = self._current_header_value
            self._header_position += 1
            self._current_header_field = None
            self._current_header_value = None
...
class HTTPSocketResponse(HTTPResponse):
    ...
    def __init__(self, sock, block_size=DEFAULT_BLOCK_SIZE, method='GET', auto_header=True):
        super(HTTPSocketResponse, self).__init__(method)
        self._sock = sock
